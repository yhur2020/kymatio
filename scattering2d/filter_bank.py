...
def filter_bank(M, N, J, L=8):
    ...
    for j in range(J):
        for theta in range(L):
            ...
            for res in range(min(j + 1, max(J - 1, 1))): # We are going to use 2 filter for  layer 1
                                                         # and 2 filters for layer 2
                if res == 0 : # filters for layer 1
                    temp = psi_signal_fourier
                    psi[res] = temp 
                if res == 1 : # filters for layer 2 = res + 1
                    psi_signal_fourier_res = periodize_filter_fft(psi_signal_fourier, res + 1)
                    psi[res] = psi_signal_fourier_res
            if j != 0 :
                psi[2] = periodize_filter_fft(psi_signal_fourier, 1) 
    ...
    # drop the imaginary part, it is zero anyway : because it's Gaussian.
    ...
    for j in range(J) :
        ...
        for lay in range(J) :
            if lay == 0 :
                phi[lay] = phi_signal_fourier 
            else : 
                phi[lay] = periodize_filter_fft(phi_signal_fourier, lay + j)
        filters['phi'].append(phi)
 ...
 #       for lay in range(J) :     
 #           if lay == 0 :
 #               phi[lay] = periodize_filter_fft(phi_signal_fourier, 1) 
 #               phi_signal_fourier = periodize_filter_fft(phi_signal_fourier, 1) 
 #               phi[lay] = phi_signal_fourier
                # not using j in layer 0 <=> No j dependency 
 #           else : 
 #               phi[lay] = periodize_filter_fft(phi_signal_fourier, lay + j + 1)
               # phi_signal_fourier = periodize_filter_fft(phi_signal_fourier, lay + j+1)
               # phi[lay] = phi_signal_fourier
 #       filters['phi'].append(phi)
...
#    for lay in range(J):
#        for j in range(J) : 
#            phi = {} # initialize and new start
#            
#            if lay == 0 :
#                phi_signal_fourier_res = periodize_filter_fft(phi_signal_fourier, 1)
#                phi[lay] = phi_signal_fourier_res
            # no ussing j <=> No j dependency
            # and also lay = 0
#            else :
#                phi_signal_fourier_res = periodize_filter_fft(phi_signal_fourier, lay + j + 1)
#                phi[lay] = phi_signal_fourier_res
...
#        filters['phi'].append(phi)
    ...
    #
    # for res in range(J) : 
    #    phi_signal_fourier_res = periodize_filter_fft(phi_signal_fourier, 1 + res*2 ) 
    #    filters['phi'][res] = phi_signal_fourier_res
    
    # supports are reduced by half for the max-pooling version.  
    return filters
...
def gabor_2d(M, N, sigma, theta, xi, slant=1.0, offset=0):
    ...
    for ex in [-2, -1, 0, 1, 2]:
        for ey in [-2, -1, 0, 1, 2]:
            ...
            gab += np.exp(arg) # smoothing...?, fourier transform...?
