...
class Pad(object):
    def __init__(self, pad_size, pre_pad=False):
        ...
        """
        self.pre_pad = pre_pad #default 값이 False
        self.padding_module = ReflectionPad2d(pad_size)

    def __call__(self, input): # __call__ 은 class가 객체로 즉 .. =class() 로 정의되지 않고
                               # 딸랑 하나 class() 처럼 함수로 call될때 실행되는 함수이다.

        if(self.pre_pad): #True 이니까 padding이 없다.
            output = input.new_zeros(input.size(0), input.size(1), input.size(2), input.size(3), 2)
            # 새로운 tensor 를 0으로 만드는데. 4차원input data 를 받아 5차원 data로 만든다. 
           
            output.narrow(output.ndimension()-1, 0, 1)[:] = input

            # narrow는 기존 tensor중에 axis 몇개를 골라 새로운 tensor를 만든다
            # 이것으로 real 과 imaginary를 구분하는 듯 하다.
            # output.ndimension()-1 : 결국 마지막 axis의 즉 가장 안쪽에 있는 데이터 = (real, imaginary)
            # 그 데이터의 0 : 1 즉 첫번째 숫자열(real)을 가져와 output에 저장한다. 
        else:
