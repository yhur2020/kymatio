...
class SubsampleFourier(object):
    ...
    def __call__(self, x, k):
        ...
        if M%k == 0 and N%k == 0  :
            ...
            # # taking average along the axis = (1,3) so the output shape is (x.shape[1]//k, x.shape[2]//k)
        else :
...
backend.fft = FFT(lambda x: tf.signal.fft2d(x, name='fft2d'), #fft2d 인데 2d가 3d 여도 해줌. 가장 안쪽 axis 두개에 대해서
