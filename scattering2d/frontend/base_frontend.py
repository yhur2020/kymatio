...
class ScatteringBase2D(ScatteringBase): # 이 ScatteringBase 까지 들어가는건 너무 깊게 보는 듯.
    def __init__(self, J, shape, L=8, max_order=2, pre_pad=False,
        ...
        self.J = J
        self.shape = shape #image 한장의 shape 
        self.max_order = max_order # =2
        self.out_type = out_type # = 'array' string임.

    def build(self): #뭔가 parameter를 build
        ...
        # compute padding은 패딩된 size를 return하는 함수
        # M_padded = [(M+2^J)/2^J]2^J + 2^J 이때 M = 28 (MNIST) J=3 일때 M_padded = 40.....double checked
        ...
        # backend.pad is in kymatio/scattering2d/backend/tensorflow_backend.py
        # it uses reflection padding and maps input to complex.
        # pre_pad = True -> no padding


        self.unpad = self.backend.unpad
        # backend.undpad is in kymatio/scattering2d/backend/tensorflow_backend.py
    ...
    # classmethod 는 후에 instanse를 만들지 않고 바로 _document 함수를 사용할 수 있게 해줌.
    @classmethod
    def _document(cls):
